...
Phase 0〜8を実装済みです。Mac/Simulator回帰に加え、Quest 3実機で映像・入力・左右26関節の
...
- Unity `6000.2.5f1`以降（検証環境: `6000.2.5f1` / `6000.3.6f1`）とAndroid Build Support（SDK / NDK / OpenJDKを含む）
...
`Window > MaQuestLink`で`OpenXR runtime JSON`を指定し、`Configure Standalone OpenXR`を押してください。

APKのbuildとinstallは初回導入またはMaQuestLink更新時だけです。通常のPlayではAndroid buildを行わず、
インストール済みQuest clientを起動してEditorの描画を即時streamします。`Window > MaQuestLink` の
`Passthrough preview`と`Show tracked hands`で、Play時のパススルーと診断用hand skeletonを切り替えられます。

### プロジェクトごとの差を設定する

**Window > MaQuestLink** はPlay前にUnity version、Standalone OpenXR、Simulator runtime、native layer、
...
- MaQuestLink windowの接続、fps、copy/encode時間、drop数、stream解像度を確認します。
...
- 対象はApple Silicon、Quest 3 / 3S、Unity 6000.2.5f1以降で検証した構成です。
