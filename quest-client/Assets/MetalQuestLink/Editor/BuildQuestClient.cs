...
namespace MetalQuestLink.QuestClient.Editor
{
    public static class BuildQuestClient
    {
        private const string ScenePath = "Assets/MetalQuestLink/Generated/QuestClient.unity";
        private const string DefaultApkPath = "Builds/MetalQuestLink.apk";

        [MenuItem("MetalQuestLink/Build Quest Client APK")]
        public static void Build()
        {
            ConfigureProject();
            BuildScene();

            var apkPath = GetCommandLineValue("-metalquestlinkOutput") ?? DefaultApkPath;
            ...
            if (report.summary.result != BuildResult.Succeeded)
            {
                throw new InvalidOperationException(
                    $"MetalQuestLink APK build failed: {report.summary.result}, errors={report.summary.totalErrors}");
            }
            Debug.Log($"METALQUESTLINK_APK_BUILT path={absolutePath} bytes={report.summary.totalSize}");
        }

        private static void ConfigureProject()
        {
            PlayerSettings.companyName = "MetalQuestLink";
            PlayerSettings.productName = "MetalQuestLink";
            PlayerSettings.bundleVersion = "0.2.0";
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Android, "com.metalquestlink.questclient");
        ...
        private static void BuildScene()
        {
            ...
            var client = new GameObject("MetalQuestLinkClient");
