...
namespace MetalQuestLink.QuestClient
{
    public sealed class QuestClientController : MonoBehaviour
    {
        ...
        private void EmitDiagnostic()
        {
            ...
            previousSent = sent;
            Debug.Log("METALQUESTLINK_DIAGNOSTIC " + JsonUtility.ToJson(value));
        }
        ...
        private void ParseAndroidExtras()
        {
            ...
            try
            {
                ...
                {
                    diagnosticMode = intent.Call<bool>("getBooleanExtra", "metalquestlink_diagnostic", diagnosticMode);
                    host = intent.Call<string>("getStringExtra", "metalquestlink_host") ?? host;
                    wifiFallbackHost = intent.Call<string>("getStringExtra", "metalquestlink_wifi_host") ?? wifiFallbackHost;
                    port = intent.Call<int>("getIntExtra", "metalquestlink_port", port);
                    passthroughMode = intent.Call<bool>("getBooleanExtra", "metalquestlink_passthrough", passthroughMode);
                    handVisualizationMode = intent.Call<bool>(
                        "getBooleanExtra", "metalquestlink_hand_visualization", handVisualizationMode);
                }
            }
            catch (Exception exception)
            {
                Debug.LogWarning($"MetalQuestLink Android extras could not be read: {exception.Message}");
            }
#endif
        }
        ...
        private void EnablePassthrough()
        {
            ...
            {
                Debug.LogWarning("MetalQuestLink passthrough underlay could not be enabled");
            }
        }
        ...
        private bool ResolvePresenter()
        {
            ...
            {
                Debug.LogError(
                    "MetalQuestLink ExternalSurfacePresenter is missing from the Quest client scene");
