...
namespace MetalQuestLink.QuestClient
{
#if UNITY_EDITOR
    [OpenXRFeature(
        UiName = "MetalQuestLink Immersive Projection",
        BuildTargetGroups = new[] { BuildTargetGroup.Android },
        Company = "MetalQuestLink",
    ...
    {
        public const string FeatureId = "com.metalquestlink.openxr.feature.immersiveprojection";
        private const string NativeLibrary = "metalquestlink_projection";
        ...
        protected override bool OnInstanceCreate(ulong instance)
        {
            ...
            {
                Debug.LogError(
                    "MetalQuestLink immersive projection requires XR_KHR_android_surface_swapchain");
                return false;
            }
            current = this;
            NativeSetInstance(instance);
            Debug.Log("MetalQuestLink immersive projection OpenXR feature enabled");
            return true;
        }
        ...
        public static AndroidJavaObject TryCreateSurface(int width, int height)
        {
            ...
            {
                Debug.LogError($"MetalQuestLink immersive projection surface failed: XrResult={result}");
                return null;
            }
        ...
        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_hook_get_instance_proc_addr")]
        private static extern IntPtr NativeHookGetInstanceProcAddr(IntPtr function);

        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_set_instance")]
        private static extern void NativeSetInstance(ulong instance);

        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_set_session")]
        private static extern void NativeSetSession(ulong session);

        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_set_app_space")]
        private static extern void NativeSetAppSpace(ulong appSpace);

        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_create_surface")]
        private static extern int NativeCreateSurface(uint width, uint height, out IntPtr surface);

        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_update_frame")]
        ...
        [DllImport(NativeLibrary, EntryPoint = "metalquestlink_projection_reset_session")]
