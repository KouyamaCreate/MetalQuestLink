...
CLIENT="${BUILD_DIR}/layer/metalquestlink_native_test"
RUN_ID="$$"
LAYER_LOG="${BUILD_DIR}/metalquestlink-layer-${RUN_ID}.log"
...
if ! XR_RUNTIME_JSON="${RUNTIME_JSON}" \
  ...
  METALQUESTLINK_LAYER_LOG="${LAYER_LOG}" \
  XR_LOADER_DEBUG=info \
    "${CLIENT}" --frames "${METALQUESTLINK_TEST_FRAMES:-120}" >"${CLIENT_LOG}" 2>&1; then
  cat "${CLIENT_LOG}"
  exit 1
fi
cat "${CLIENT_LOG}"

rg -q "loaded instance for MetalQuestLinkNativeTest" "${LAYER_LOG}"
rg -q "METALQUESTLINK_FRAME_LOOP_OK" "${CLIENT_LOG}"
