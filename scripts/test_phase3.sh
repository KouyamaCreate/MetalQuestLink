...
if ! METALQUESTLINK_PORT="${PORT}" METALQUESTLINK_VERIFY_INPUT=1 METALQUESTLINK_REQUIRE_HANDS=1 \
  METALQUESTLINK_TEST_FRAMES=240 \
...
rg -q "METALQUESTLINK_INPUT_E2E_OK views=1 actions=1 space=1" "${PRODUCER_LOG}"
rg -q "METALQUESTLINK_VIDEO_E2E_OK.*input_sent=[1-9][0-9]*.*clock_sync=1" "${VIEWER_LOG}"
rg -q "METALQUESTLINK_INPUT_E2E_OK.*hands=1.*haptics=1" "${PRODUCER_LOG}"
rg -q "METALQUESTLINK_VIDEO_E2E_OK.*haptic_apply=1.*haptic_stop=1.*passthrough=1.*passthrough_alpha=0.82" "${VIEWER_LOG}"
