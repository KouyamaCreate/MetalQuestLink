# MaQuestLink 実装プラン（GPT-5.6 実行用プロンプト）
...
あなたは自律的に動くシニアXRシステムエンジニア。`/Users/koseiyamamoto/Documents/GitHub/MaQuestLink`（git初期化済み・ほぼ空）で、Apple Silicon Mac 上の Unity エディタPlayモードを Meta Quest 3 実機でリアルタイムにプレビュー・プレイできるシステム「MaQuestLink」を作り切る。

作業は下記の **Phase 0〜7 を順番に** 進める。各フェーズの受け入れ基準を検証コマンドで確認してからコミットし、`docs/progress.md` にフェーズの状態（完了/未完了、検証結果、判明した事実）を追記して次へ進む。途中で実行が中断されても、`docs/progress.md` を読めば次のフェーズから再開できる状態を常に保つ。
...
5. **Unityエディタ統合パッケージ**（`com.maquestlink.editor`、ローカルUPMパッケージ）:
...
**成果物**: `editor-package/`（`com.maquestlink.editor` ローカルUPMパッケージ: レイヤー登録、エディタウィンドウ、adb reverse自動設定、APKインストール・起動ボタン、再生開始時の自動接続）、`samples/`（Meta XR SDK の最小シーン: OVRCameraRig + 掴めるキューブ）。
...
- **UPMパッケージの自己完結化**: `com.maquestlink.editor` に、ビルド済み arm64 dylib（APIレイヤー）とレイヤーマニフェストを同梱する。パッケージ導入だけでレイヤー登録・接続・APK導入ボタンまで完結し、利用者にcmake/Xcode/Homebrewを要求しない。git URL 経由（`https://... .git?path=editor-package`）とローカルtarballの両方でインストールできる構成にする。
...
- パススルー、シーンアンカー、ハンドトラッキング等のMeta固有機能はスコープ外（シミュレータの挙動のまま）。READMEに制約として明記する。ハンドトラッキングは将来課題としてプロトコルに拡張余地だけ残す。
