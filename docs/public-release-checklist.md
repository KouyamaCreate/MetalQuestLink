...
- [ ] Descriptionとtopics（例: `unity`, `openxr`, `meta-quest`, `xr`, `developer-tools`）を設定する。
- [ ] Default branchを確認し、保護ルールまたはrulesetでPull Requestと必要なCIを要求する。
- [ ] `v0.2.0` tagとGitHub Releaseを、同じcommitのUPM tarball / APK / `SHA256SUMS`付きで公開する。

## Community and security

- [ ] Issuesを有効にし、Issue Formが表示されることを確認する。
- [ ] Discussionsを有効にし、setup質問をIssueから分離する。
- [ ] **Settings > Security > Private vulnerability reporting**を有効にする。
...
- [ ] Release checksumを検証し、ad-hoc署名・未公証であることをRelease notesに明記する。
